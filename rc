...
function git-srv-init () {
	cmd1="mkdir -p /srv/git/repos/$1.git"
	cmd2="cd /srv/git/repos/$1.git && git init --bare"
	ssh git@nik-srv $cmd1&&
	ssh git@nik-srv $cmd2&& echo Done.
}
function git-srv-add-remote () {
	git remote add nik-srv git@nik-srv:/srv/git/repos/$1.git
}
...
function git-rpi-add-remote () {
	git remote add nik-srv git@nik-rpi:/srv/git/repos/$1.git
}
function cd (){
	case $1 in
		..)
